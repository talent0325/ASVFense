...
'''
通过频谱域的 U-Net 结构和增强模块 (DConv, FTB)，实现高质量信号重建
'''
...
class Aero(nn.Module):
    ...
    def __init__(self,
        ...
        for index in range(self.depth):
            ...
            # print(chin_z,chout_z) # (1,48) (48,96) (96,192)
    ...
    def _spec(self, x, scale=False):
        ...
        # z = spectro(x, nfft, hl, win_length=win_length)
        return z
    ...
    # 返回幅度谱
    def _move_complex_to_mag(self, z):
        B, C, Fr, T = z.shape
        m = torch.view_as_real(z)
        real_part = m[..., 0]
        imag_part = m[..., 1]
    ...
    def forward(self, lr_mag, return_spec=False, return_lr_spec=False):

        # afpm filtered
        x = lr_mag   # torch.Size([16, 1, 257, 501])
        x = x[:, :, :-1, :]
        # raw_mag = hr_mag[:, :, :-1, :]
        ...
        # print('after decoder')
        # print(x.shape)  # torch.Size([16, 1, 256, 501])
        ...
        # torch.Size([16, 1, 2, 256, 501]) -> torch.Size([16, 1, 256, 501])
        # x_spec_complex = self._convert_to_complex(x)    

        # if self.debug:
        #     logger.info(f'x_spec_complex shape: {x_spec_complex.shape}')

        # x = self._ispec(x_spec_complex)   # torch.Size([16, 1, 64000])
