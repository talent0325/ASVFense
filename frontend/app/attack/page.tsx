...
export default function AttackPage() {
  ...
  const handleAudioUrlChange = (url: string | null, id?: number) => {
    setAudioUrl(url);
    if (id) {
      setAudioFileId(id);
    } else {
        ...
        <div className="grid grid-cols-2 gap-0 max-w-[960px] mx-auto">
          ...
          <div className="bg-transparent rounded-xl p-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              {/* 生成对抗样本后的结果 */}
              {showResult && attackResult && (
                <div className="w-full">
                  <h2 className="text-[28.8px] font-bold text-[#655DE6] mb-4">攻击音频生成结果</h2>
                  <div className="w-full flex items-center gap-4">
                    <audio src={`http://localhost:5000/api/audio/${attackResult.id}`} controls className="w-[80%]" />
                    <button 
                      onClick={() => {
                        // 创建下载链接
                        const link = document.createElement('a');
                        // TODO: 写死了这里
                        link.href = `http://localhost:5000/api/audio/${attackResult.id}`;
          ...
          <div className="bg-transparent rounded-xl p-6">
            ...
            <AttackFormContent 
              ...
              onGenerate={handleGenerate}   // 生成成功后回调函数，由父组件定义
              buttonText={buttonText}     // 按钮文字，可以自定义
              isButtonDisabled={!audioFileId}  //如果没有上传或录制音频，则按钮禁用
              audioFileId={audioFileId || undefined}   // 传入当前音频的 ID，如果不存在则传 undefined，保证子组件可以判断是否有音频可用
            />
          </div>
        </div>
      </div>
