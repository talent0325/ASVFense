...
export default function AttackModal({ isOpen, onClose, onAttackAudioSelected }: AttackModalProps) {
  ...
  const [audioFileId, setAudioFileId] = useState<string | undefined>(undefined);
  ...
  const handleGenerate = () => {
    if (buttonText === '攻击音频生成') {
      ...
      if (audioUrl && onAttackAudioSelected) {
        const originalFilename = audioFileId?.split('_', 1)[1] || audioFileId || '';
        onAttackAudioSelected(audioUrl, {
          ...attackInfo,
          audioFileId: originalFilename
        });
      }
      onClose();
    }
  ...
  const handleAudioUrlChange = (url: string | null, id?: string) => {
    setAudioUrl(url);
    setAudioFileId(id || undefined);
  ...
  const handleAttackInfoChange = (info: {
    ...
    setAttackInfo({
      isAdaptive: info.attack_info.is_adaptive,
      hasTarget: info.attack_info.has_target,
      attackType: info.attack_type,
      targetSpeaker: info.target_speaker,
      asvModel: info.asv_model
    });
  };
