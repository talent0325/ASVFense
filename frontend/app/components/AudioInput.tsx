...
interface AudioInputProps {  // 当上传或录音完成后，调用父组件传入的回调，传回音频的 URL 和后端返回的 id
  onAudioUrlChange: (url: string | null, id?: string) => void;
  onReset?: () => void;  // 重置用的回调
}
...
export default function AudioInput({ onAudioUrlChange, onReset }: AudioInputProps) {
  ...
  const [hasAudio, setHasAudio] = useState(false);   // 是否已有音频（上传/录制成功）
  const [mediaRecorder, setMediaRecorder] = useState<MediaRecorder | null>(null);   // MediaRecorder 实例
  const [audioChunks, setAudioChunks] = useState<Blob[]>([]);   // 录音过程中分段存储的音频数据
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {   //上传音频文件到后端，后端返回 id，再拼接成可访问的 URL
    ...
    if (file) {
      ...
      try {
        ...
        const audioUrl = `http://localhost:5000/api/audio/${data.id}`;
        onAudioUrlChange(audioUrl, data.id);
        setHasAudio(true);
      } catch (error) {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          // TODO: 如果后端写成 request.data 或只支持 JSON，那会直接报错 Flask 示例 file = request.files['audio']
          const data = await response.json();
          const audioUrl = `http://localhost:5000/api/audio/${data.id}`;
          onAudioUrlChange(audioUrl, data.id);
          setHasAudio(true);
          ...
          alert('录音上传失败，请重试');  // 如果前端在 http://localhost:3000，后端在 http://localhost:5000，没开 CORS 会导致上传失败。
        }
  ...
  const handleReset = () => {  //清除状态，并通知父组件“当前没有音频了
    setHasAudio(false);
    setIsRecording(false);
    ...
    <div className="flex flex-col gap-8">
      {!hasAudio && (
        <>
          ...
          <div className="relative w-[380px] h-[180px]">
            <div className="absolute inset-0 bg-gradient-to-r from-[#B224EF] to-[#7579FF] opacity-22 blur-[10px] rounded-lg"></div>
            {/* 一个点击区域，点下去会触发隐藏的 <input type="file">。录音卡片 根据 isRecording 状态切换显示，录音时有红点闪烁动画*/}
